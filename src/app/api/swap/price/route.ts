...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL(`${ZERO_EX_BASE_URL}/swap/allowance-holder/price`);
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, error: extract0xError(data), details: data },
    ...
    console.error("GET /api/swap/price:", err);
    return NextResponse.json({ success: false, error: "Swap price request failed" }, { status: 500 });
  }
}
