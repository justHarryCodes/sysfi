  ...
  if (!initialized)
    return (
      <GlassCard className="p-12 text-center">
        ...
        <Link
          href="/"
          className="inline-block mt-4 text-neon-blue font-mono text-sm hover:underline"
        >
          ← Back
        </Link>
      </GlassCard>
  ...
  return (
    <div className="animate-fade-in space-y-5">
      <Link
        href="/"
        prefetch
        className="inline-flex items-center gap-1.5 text-sm font-mono text-text-secondary hover:text-neon-blue transition-colors"
      >
      ...
      >
        {meta?.hasBanner ? (
          ...
          <img
            src={imageUrl(poolAddr, chainId, "banner")}
            alt="banner"
          ...
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${chainMeta.bgColor} 0%, rgba(0,255,135,0.06) 50%, ${chainMeta.bgColor} 100%)`,
            }}
          >
            <div
        ...
        {graduated && (
          <div
            className="absolute top-3 right-3 px-3 py-1 rounded-xl text-xs font-mono font-bold backdrop-blur"
            style={{
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {meta?.hasLogo ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={imageUrl(poolAddr, chainId, "logo")}
                  alt={displaySymbol}
                  className="w-full h-full object-cover"
                />
              ) : (
          ...
          {(
            ...
            ({ label, addr }) =>
              addr && (
                <div
                  key={label}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-black/30 border border-white/5"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-4">
            ...
            {[
              {
                label: "Price",
                primary:
                  priceWei != null ? `${formatPrice(priceWei)} ${native}` : "—",
                secondary: priceUSD > 0 ? formatUSD(priceUSD) : null,
              },
              {
                label: "FDV",
                primary: fdv || "—",
              ...
              },
              {
                label: "Raised",
                primary:
                  poolETH != null ? `${formatETH(poolETH)}  ${native}` : "—",
                secondary: raisedUSD > 0 ? formatUSD(raisedUSD) : null,
              },
              {
                label: "Fees",
                primary:
                  feesETH != null ? `${formatETH(feesETH)}  ${native}` : "—",
                secondary: null,
              },
            ].map(({ label, primary, secondary, highlight }) => (
              <GlassCard
                key={label}
                glow={highlight ? "green" : "none"}
                className="p-3"
              >
          ...
          <PriceChart
            poolAddr={initialized ? (poolAddr as `0x${string}`) : undefined}
            currentPriceWei={priceWei ?? 0n}
          />
